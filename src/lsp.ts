...
/** 一条在途 LSP 请求的 settle 钩子（resolve 回包 / reject 超时或服务器退出）。 */
...
export class LspManager {
  ...
  async query(action: 'diagnostics' | 'hover' | 'definition' | 'references', path: string, line?: number, column?: number): Promise<{ text: string }> {
    ...
    if (this.serverDead(state)) return { text: `${lang} 语言服务器已退出` }
    ...
    return await withTimeout(
      this.queryInner(lang, state, action, path, realPath, uri, line, column),
      LSP_QUERY_TIMEOUT_MS,
      () => ({ text: `LSP 查询超时（${LSP_QUERY_TIMEOUT_MS / 1000}s）：${lang} 语言服务器无响应，可能未启动或已退出` }),
    )
  }
  ...
  private serverDead(state: ServerState): boolean {
   ...
   */
  private rejectPending(state: ServerState, err: Error): void {
    for (const [, entry] of state.pending) entry.reject(err)
    state.pending.clear()
  }
