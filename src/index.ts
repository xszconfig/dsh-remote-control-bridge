...
export async function apply(ctx: Context) {
  ...
  const controller = new ReloadController({
    ...
    applyPlugin: async (mod) => {
      ...
      const fiber = ctx.plugin(mod as Plugin.Object)
      await fiber
      return fiber
    },
  })
  ...
  const reloadRoute: WebRoute = {
    ...
    handler: async (req, res) => {
      ...
      if (result === 'busy') return json(res, { ok: false, error: 'busy' }, 409)
      if (result === 'failed') return json(res, { ok: false, error: controller.lastError ?? 'reload failed' }, 500)
      return json(res, { ok: true, ...controller.status() })
    },
  }

  const hotRoute: WebRoute = {
    kind: 'exact',
    path: '/remote/hot',
    handler: (req, res) => {
      if (!allowLocalOrEnvToken(req, res, envToken)) return denied(res)
