...
import {
  ...
  type GoalWire,
  type LogEntryWire,
  type ModelCatalogFailureWire,
  type ModelProviderGroupWire,
  type ModelSelectionWire,
  type PairInfo,
...
export function apply(ctx: Context) {
  ...
  /** LLM 服务软类型（模型目录读取 + 切换；headless 部署可缺省 → 目录/切换降级）。 */
  interface LlmLike {
    listProviders(): { id: string; name: string }[]
    listModels(providerId: string): Promise<{ id: string; name: string; description?: string }[]>
    resolveModelInfo(
      providerId: string,
      modelId: string,
    ): Promise<{
    ...
    ): Promise<{ provider: string; model: string; reasoningEffort?: ReasoningEffortId }>
  }
  /** 投影注册表软类型（contextPressure/contextBreakdown 读取 + change feed 订阅）。 */
  ...
  const selectionInstalled = new WeakSet<Agent>()
  ...
  const modelsWireOf = async (sessionId: string): Promise<SessionModelsWire> => {
    ...
    const current: ModelSelectionWire | null = a !== undefined ? modelSelectionOf(a).current ?? null : null
    if (llm === undefined || typeof llm.listProviders !== 'function') {
      // 无 llm 服务：无目录、无法判 routable → current 保留、其余空/unknown
      return { current, routable: null, groups: [], failures: [] }
    }
    const providers = llm.listProviders()
    const groups: ModelProviderGroupWire[] = []
    const failures: ModelCatalogFailureWire[] = []
  ...
  const pendingContextUsage = new Set<string>()
  const scheduleContextUsage = (sid: string): void => {
    if (pendingContextUsage.has(sid)) return
  ...
  if (projections?.onChanged !== undefined) {
    projections.onChanged((session, key) => {
      if (key === 'contextPressure' || key === 'contextBreakdown') {
        scheduleContextUsage(String((session as { id: unknown }).id))
      }
    })
  }
