...
export function apply(ctx) {
    ...
    /** 子代理会话发消息：先拉起父会话，再经官方 ctx.subagents.followup 冷恢复并投递。 */
            ...
            case 'send_message': {
                ...
                if (a === undefined) {
                    ...
                    if (header !== undefined && (header.origin === 'subagent' || header.parentSession !== undefined)) {
                        await deliverSubagent(ws, cmd, header);
                        break;
                    }
...
// ---- pairing helpers ----
function buildPairInfo(req, pairToken, port, bindHost) {
    const ips = lanIpv4s();
    ...
    if (bindHost === '0.0.0.0') {
        for (const ip of ips)
            candidates.push(ip);
        if (headerHost && !candidates.includes(headerHost))
            candidates.push(headerHost);
    }
    else {
