...
import { loadDeliveries, writeDeliveries } from './deliveries.js';
...
function saveDevices(devices) {
    ...
    try {
        mkdirSync(dshHome, { recursive: true });
        const payload = { version: 1, devices };
...
export function apply(ctx) {
    ...
    if (!envToken) {
        console.log(`[dsh-remote-control-bridge] no DSH_REMOTE_TOKEN set — clients may connect unauthenticated ` +
            `(bridge binds to the loopback web server only).`);
    }
    const clients = new Set();
    let devices = loadDevices();
    const pairTokens = new Map(); // token -> expiry epoch ms
    const thinkStreams = new Map(); // sessionId -> 思考流累积
    const pairPrune = setInterval(() => {
        const now = Date.now();
        for (const [t, exp] of pairTokens)
            if (exp < now)
                pairTokens.delete(t);
    }, 60_000);
    pairPrune.unref?.();
    ...
    if (bootWork !== null && (bootWork.activity !== null || bootWork.pending.length > 0)) {
        ...
        for (const p of bootWork.pending)
            logger.info('WORK', `  - ${p}`);
    }
    else {
    ...
    const upsertDevice = (deviceId, name, model) => {
        const now = Date.now();
    ...
    const deviceByToken = (token) => devices.find((d) => d.token === token);
    // ---- helpers ----
    const allAgents = () => ctx.agents.list();
    const agentOf = (sessionId) => allAgents().find((a) => String(a.id) === sessionId);
    /** 会话实际运行的 preset：后选优先，无事件回退 header（与 dsh-agent-presets 同款）。 */
    const sessionPresetOf = (session) => {
        for (let i = session.events.length - 1; i >= 0; i -= 1) {
            const ev = session.events[i];
            if (ev !== undefined && ev.type === 'agent-preset/selected') {
                return ev.data?.agentPreset;
            }
        }
        return session.header.agentPreset;
    };
    /** per-agent 可变模型选择 ref（同 apiproxy 的 `selections`：WeakMap 复用同一 ref，切换即生效）。 */
    const selections = new WeakMap();
    /** 已挂载 installModelSelection 的 agent（防重复挂载瀑布监听）。 */
    const selectionInstalled = new WeakSet();
    /** 复刻 apiproxy selectionFor：优先会话最近记录模型，回退 agentDefaultModel；set current 供手机端切换。 */
    const modelSelectionOf = (agent) => {
    ...
    const installSelectionFor = (agent, scopedCtx) => {
    ...
    const sessionHeaderOf = async (sessionId) => {
    ...
    const resumeAgent = (sessionId) => {
        ...
        if (live !== undefined)
            return Promise.resolve({ agent: live });
            ...
            try {
                ...
                const handle = await ctx.agents.resume({
                    ...
                    setup: async (agentCtx) => {
                        ...
                        if (agent === undefined)
                            throw new Error('dsh-remote-control-bridge: resume setup 缺少 scoped agent');
                        // 1) 模型选择（会话最近记录 → agentDefaultModel 兜底；可变 ref，供 set_model 切换），与 Web 的 installSelection 对齐
                        installSelectionFor(agent, agentCtx);
                        // 2) 挂载 preset（有 agentPresets 服务时）：恢复该会话的工具/提示词/目标等能力
                        if (presets !== undefined)
                            await presets.mount(agentCtx, sessionPresetOf(agent.session));
                    },
                });
    ...
    const deliverSubagent = async (ws, cmd, header) => {
        ...
        try {
            await subagents.followup(parentRes.agent, SessionId(cmd.sessionId), [{ type: 'text', text: cmd.text }], { source: { kind: 'user' }, signal: new AbortController().signal });
    ...
    const goalWireFromProjection = (p) => {
        const g = p?.goal;
    ...
    const goalWireOf = (sessionId) => {
        try {
            ...
            if (view !== undefined)
                return goalWireFromView(view);
            const proj = ctx.get('sessionProjections');
            return goalWireFromProjection(proj?.snapshot(session).values?.['goal']);
        }
        catch (e) {
    ...
    const todosOfSession = (session) => {
        try {
            const proj = ctx.get('sessionProjections');
            const todos = proj?.snapshot(session).values?.['todos'];
     ...
     */
    const subagentLabelFromValues = (values) => {
     ...
     */
    const runDurationMsFromValues = (values) => {
     ...
     */
    const totalTokensFromValues = (values) => {
    ...
    /**
     * 上下文窗口占用 wire（对齐 DSH Web contextPressure + contextBreakdown）：
     ...
     * 无 provider 上报 usage / 无容量 → 对应字段缺省，客户端不渲染占用环。
     */
    const contextUsageFromValues = (values) => {
    ...
    const contextUsageWireOf = (s) => contextUsageFromValues(s === undefined ? undefined : liveProjectionValues(s));
    /**
     ...
     */
    const contextUsageWithDefault = async (sessionId, base) => {
        if (base.percent !== undefined)
            return base;
        try {
     ...
     */
    const inheritedSelectionAgentOf = async (sessionId) => {
        const seen = new Set();
        let cur = sessionId;
    ...
    const modelsWireOf = async (sessionId) => {
        ...
        const current = effectiveAgent !== undefined ? modelSelectionOf(effectiveAgent).current ?? null : null;
        if (llm === undefined || typeof llm.listProviders !== 'function') {
            // 无 llm 服务：无目录、无法判 routable → current 保留、其余空/unknown
            return { current, routable: null, groups: [], failures: [] };
        }
            ...
            try {
                ...
                const entries = await Promise.all(models.map(async (m) => {
                    ...
                    return {
                        ...
                        ...(reasoning === undefined ? {} : {
                            reasoning: {
                                efforts: reasoning.efforts.map((e) => ({ id: e.id, name: e.name, ...(e.description === undefined ? {} : { description: e.description }) })),
                ...
                failures.push({ id: provider.id, name: provider.name, message: e instanceof Error ? e.message : String(e) });
            }
        }
    ...
    let userInvocableSkillNames = new Set();
    const skillsWireOf = async () => {
        ...
        try {
            ...
            // 只读全局层会漏掉本地技能（此前只返回 1 个的根因）；须带 scope（首个 live agent）+ 其 cwd。
            const live = ctx.agents.list()[0];
    ...
    const listWorkspaces = () => ctx.workspaceRegistry.list().map((w) => ({
        id: String(w.id),
        title: w.title,
        path: w.path,
        sessionCount: w.sessionIds.length,
    }));
    const workspaceIdOf = (sessionId) => {
        for (const w of ctx.workspaceRegistry.list()) {
            if (w.sessionIds.some((sid) => String(sid) === sessionId))
                return String(w.id);
        }
        return null;
    };
    /** 活跃会话的行投影（agent 未挂载时为 idle）。 */
    const sessionRowFromLive = (s) => {
        ...
        return {
            ...
            agentCount: a ? 1 : 0,
            subagentCount: allAgents().filter((x) => String(x.session.header.parentSession) === id && isSubagent(x)).length,
            updatedAt,
    ...
    const coldRowFromCache = (h) => {
        ...
        try {
            ...
            if (typeof meta?.lastPromptAt === 'number')
     ...
     */
    let sessionsMemo = null;
    const listSessions = async () => {
        const now = Date.now();
        if (sessionsMemo && now - sessionsMemo.at < 5_000)
            return sessionsMemo.rows;
    ...
    const listAgents = () => allAgents().map((a) => ({
    ...
    const debug = new DebugManager({
        ...
        onVariables: (sessionId, variablesReference, variables) => broadcast({ type: 'debug_variables', sessionId, variablesReference, variables }),
    });
            ...
            execute: async (args) => {
                ...
                try {
                    if (a.action === 'variables') {
                        ...
                        const vars = await withTimeout(debug.variablesFor(a.sessionId, a.variablesReference), 10_000, () => undefined);
                        if (vars === undefined)
                            return { text: '调试读变量超时（10s）：调试器无响应，可能已停止/断开' };
                        return {
                            text: vars.length === 0
                                ? '（无变量，或引用已随恢复失效）'
                    ...
                    if (a.action === 'stop') {
                        const stopped = await withTimeout(debug.stop(a.sessionId).then(() => true), 10_000, () => false);
    ...
    const projectEvents = (events, scope) => events.flatMap((event) => projectEvent(ctx, event, scope));
    /**
     * 冷会话原始事件缓存：翻页复用，避免每页都重读/重解析整份日志。
     * TTL 5 分钟，最多缓存 5 个会话（超出按最旧淘汰）。
     */
    const coldEventCache = new Map();
    const coldSessionEvents = async (sessionId) => {
        const hit = coldEventCache.get(sessionId);
        if (hit !== undefined && Date.now() - hit.at < 5 * 60_000)
            return hit.events;
        if (!ctx.sessionPersistence)
            throw new Error('no sessionPersistence service');
    ...
    const queueSnapTimers = new Map();
    const writeQueueSnapshot = (sessionId, items) => {
        try {
    ...
    const flushQueueSnapshot = (sessionId) => {
    ...
    const scheduleQueueSnapshot = (sessionId, items) => {
    ...
    const restoreQueueIfLost = (sessionId) => {
        ...
        try {
            ...
            if (snap === undefined || snap.items.length === 0)
                return;
            const agent = agentOf(sessionId);
            if (agent === undefined)
                return; // 会话未挂载：保留快照，等 agent 上线再试
            const live = userQueueItemsOf(agent);
            const missing = snap.items.filter((it) => !live.some((l) => l.id === it.id || l.text === it.text));
            if (missing.length === 0) {
    ...
    const resumeFingerprintOf = (work) => `${work.activity ?? ''}\n${work.pending.join('\n')}\n${work.sessionId ?? ''}`;
    ...
    const tryResumeIfPending = () => {
        try {
            if (process.env.DSH_REMOTE_RESUME === '0')
                return;
            if (resumeInjecting)
                return; // 同一批待办正在注入（含 followup 同步触发的 agent/status 重入）
            const work = loadWorkState(WORK_FILE);
            if (work === null || work.pending.length === 0)
                return;
            const fp = resumeFingerprintOf(work);
            if (work.resumeFingerprint === fp)
                return; // 这批待办已注入过：等 agent 更新文件后再唤醒，不重复打扰
            const live = ctx.agents.list();
            let agent;
            if (work.sessionId !== undefined)
                agent = agentOf(work.sessionId);
            if (agent === undefined)
                agent = live.find((a) => !isSubagent(a)) ?? live[0];
            if (agent === undefined) {
    ...
    const resumeTick = () => {
        ...
        try {
            const work = loadWorkState(WORK_FILE);
            for (const sid of Object.keys(work?.queues ?? {}))
                restoreQueueIfLost(sid);
        }
        catch (e) {
    ...
    let deliveryRecords = loadDeliveries(DELIVERIES_FILE);
    // turnOutput：会话 → 本轮是否有「最终结论(assistant) / 非空工具产出(tool)」，降噪用（turn/start 重置）。
    const turnOutput = new Map();
    ...
    const commandWireList = (agent) => {
        const commands = ctx.get('commands');
        if (agent === undefined || commands?.list === undefined)
            return [];
        try {
    ...
    ctx.on('llm/stream', (options, next) => {
        ...
        return (async function* () {
            try {
                ...
                if (modelStreams.get(sid) === startedAt)
                    modelStreams.delete(sid);
     ...
     */
    const maybeDeliverOnTurnEnd = (session) => {
        try {
    ...
    const trackTurnOutput = (to, proj) => {
        if (proj.type === 'assistant_message')
            to.assistant = Date.now();
    ...
    ctx.on('session/event', (session, event) => {
        ...
        for (const proj of projectEvent(ctx, event, scope)) {
            ...
            if (to !== undefined)
                trackTurnOutput(to, proj);
        }
        // LSP：实时事件里 Agent 编辑/写入文件 → 触发语言诊断（历史投影不触发，避免重放风暴）
        if (event.type === 'tool/call') {
            for (const p of toolFilePaths(event.data))
                lsp.notifyFileChanged(p, String(session.id));
        }
    });
    ...
    // 上下文占用投影变更 → 推 context_usage（sessionProjections change feed，事件驱动非轮询）。
    // 批处理：同一事件里 contextPressure + contextBreakdown 可能同时变，microtask 合并成一条；
    // 且 change feed 在投影 apply 后触发，快照已是落库后状态。
    const pendingContextUsage = new Set();
    const scheduleContextUsage = (sid) => {
        if (pendingContextUsage.has(sid))
            return;
        pendingContextUsage.add(sid);
        queueMicrotask(() => {
            pendingContextUsage.delete(sid);
            try {
                const s = ctx.sessions.list().find((x) => String(x.id) === sid);
                broadcast({ type: 'context_usage', sessionId: sid, usage: contextUsageWireOf(s) });
            }
            catch (e) {
                logger.warn('CTX', `上下文占用投影广播失败 session=${sid.slice(0, 12)}: ${String(e)}`);
            }
        });
    };
    ...
    if (projections?.onChanged !== undefined) {
        projections.onChanged((session, key) => {
            if (key === 'contextPressure' || key === 'contextBreakdown') {
                scheduleContextUsage(String(session.id));
            }
        });
    }
    ...
    const pendingApprovalList = () => [...pendingApprovals.values()].map(({ resolve: _r, timer: _t, ...wire }) => wire);
    /** 与桌面端相同的审计关联：从会话日志找到最新一条未裁决且 callId 匹配的 approval/asked。 */
    const approvalIdOf = (req) => {
    ...
    const commandOfCall = (session, callId) => {
        for (let i = session.events.length - 1; i >= 0; i -= 1) {
    ...
    const muxRemoteApprovals = new Map();
    /** 桌面端持有、经 mux 转发的提问（rpcId -> wire）。 */
    const muxQuestions = new Map();
    let muxStopped = false;
    let muxWs = null;
    /** 经 /api/respond 回传手机裁决给桌面端 answerer（返回是否被接受）。 */
    const respondToDesktop = async (rpcId, value) => {
        try {
    ...
    const handleMuxFrame = (envelope) => {
        ...
        switch (payload.type) {
            case 'approval/requested': {
                const f = payload;
                if (pendingApprovals.has(f.approvalId))
                    return; // 本 bridge 已持有（防御性去重）
        ...
        // 缓存过期清扫（30 分钟未解决视为失效）
        const now = Date.now();
        for (const [id, wire] of muxRemoteApprovals)
            if (now - wire.requestedAt > 30 * 60_000)
                muxRemoteApprovals.delete(id);
        for (const [id, wire] of muxQuestions)
            if (now - wire.requestedAt > 30 * 60_000)
                muxQuestions.delete(id);
    };
    /** 维护与 /api/events.mux 的 WebSocket 长连接（只读下链，断开自动重连）。 */
    const runMuxClient = async () => {
        let failures = 0;
        // 帧类型计数（节流日志用）
    ...
    const authenticate = (req) => {
        ...
        if (envToken) {
            if (candidates.includes(envToken))
                return { kind: 'env' };
        }
    ...
    const handleCommand = async (ws, raw) => {
        let cmd;
        try {
        ...
        const extra = [
            'sessionId' in cmd && typeof cmd.sessionId === 'string'
                ? `session=${cmd.sessionId.slice(0, 12)}`
                : '',
            'approvalId' in cmd && typeof cmd.approvalId === 'string'
                ? `approval=${cmd.approvalId.slice(0, 8)}`
                : '',
            'rpcId' in cmd && typeof cmd.rpcId === 'string'
                ? `rpc=${cmd.rpcId.slice(0, 8)}`
                : '',
            cmd.type === 'send_message' && typeof cmd.text === 'string'
                ? `text=${cmd.text.slice(0, 40)}`
                : '',
        ].filter(Boolean).join(' ');
        ...
        switch (cmd.type) {
            ...
            case 'upload_logs': {
                ...
                while (phoneLogBuffer.length > 5000)
                    phoneLogBuffer.shift();
                logger.info('PHONE', `收到手机回传日志 ${entries.length} 条 (request=${cmd.requestId?.slice(0, 8) ?? '-'})`);
                break;
            }
            case 'queue_action': {
            ...
            case 'subscribe': {
                ...
                if (liveSession) {
                    ...
                    void contextUsageWithDefault(String(liveSession.id), contextUsageWireOf(liveSession)).then((usage) => send(ws, { type: 'context_usage', sessionId: String(liveSession.id), usage }));
                ...
                try {
                    ...
                    try {
                        ...
                        goal = goalWireFromProjection(values?.['goal']);
                    ...
                    // 冷会话：上下文占用走投影缓存冷读（子会话/顶层共用），无数据时兜底补默认 percent=0；模型目录仍可列（current=父会话继承）
                    {
                        ...
                        void contextUsageWithDefault(sid, contextUsage).then((usage) => send(ws, { type: 'context_usage', sessionId: sid, usage }));
            ...
            case 'send_message': {
                ...
                if (msgId !== undefined) {
                    ...
                    if (prior !== undefined)
                        processedMsgIds.delete(msgId); // 过期 → 视为未处理，重新投递
                }
                try {
            ...
            case 'interrupt': {
                ...
                if (mode === 'keep') {
                    ...
                    a.cancel({ kind: 'user' });
                    for (const m of keep)
                        a.followup(createUserMessage({ content: m.content, source: m.source }));
            ...
            case 'set_model': {
                // 鉴权：模型切换是会话控制命令，需已认证连接（env/pair/device/open 均可；none 已在上游 authenticate 拦截）
                const auth = wsAuth.get(ws);
                if (auth !== undefined && auth.kind === 'none') {
                    send(ws, { type: 'error', code: 'forbidden', message: 'not authorized to switch model' });
                    break;
                }
                const sid = cmd.sessionId;
                let a = agentOf(sid);
                if (a === undefined) {
                    const found = await resumeAgent(sid);
                    if (found.agent === undefined) {
                        send(ws, { type: 'error', code: 'not_running', message: `该会话当前未在桌面端打开，且自动打开失败：${found.error ?? '未知原因'}` });
                        break;
                    }
                    a = found.agent;
                }
                ...
                if (llm === undefined || typeof llm.resolveCallConfig !== 'function') {
                    send(ws, { type: 'error', code: 'model_unavailable', message: '该部署未挂载 LLM 服务，无法切换模型' });
                    break;
                }
                try {
                    const resolved = await llm.resolveCallConfig({
                        ...
                        model: cmd.model,
                        ...(cmd.reasoningEffort === undefined ? {} : { reasoningEffort: ReasoningEffortId(cmd.reasoningEffort) }),
                    });
                    const selected = {
                    ...
                    // 下一步 prompt 组装边界生效，不打断当前推理；仅当前会话生效、不存部署默认（已拍板）
                    installSelectionFor(a).current = selected;
                    logger.info('MODEL', `切换模型 session=${sid.slice(0, 12)} provider=${selected.provider} model=${selected.model}${selected.reasoningEffort !== undefined ? ` effort=${selected.reasoningEffort}` : ''}`);
                    // 回 models_update 给手机端确认（当前选择已更新）
                    const models = await modelsWireOf(sid);
                    broadcast({ type: 'models_update', sessionId: sid, models });
                }
                catch (e) {
                    send(ws, { type: 'error', code: 'model_unavailable', message: e instanceof Error ? e.message : String(e) });
                }
                break;
            }
            case 'approve': {
            ...
            case 'revoke_device': {
                const auth = wsAuth.get(ws) ?? { kind: 'open' };
                const mayRevoke = auth.kind === 'env' || auth.kind === 'pair' || auth.kind === 'open' || auth.deviceId === cmd.deviceId;
                if (!mayRevoke) {
            ...
            default: {
                send(ws, { type: 'error', code: 'unknown_command', message: `unknown command: ${cmd.type}` });
            }
        }
    };
    // ---- WebSocket upgrade ----
    ...
    wss.on('connection', (ws, req) => {
        ...
        ws.on('message', (data) => void handleCommand(ws, data));
        ws.on('close', () => {
            clients.delete(ws);
    ...
    const ping = {
        kind: 'exact',
        path: '/remote/ping',
        handler: async (_req, res) => json(res, { ok: true, version: BRIDGE_VERSION, serverId, hostname: host, sessions: (await listSessions()).length }),
    };
    const health = {
        kind: 'exact',
        path: '/remote/health',
    ...
    const questionTestRoute = {
        ...
        handler: async (req, res) => {
            ...
            const sessionId = String(parsed.sessionId ?? allAgents().find((a) => a.status === 'running')?.id ?? 'debug-question-session');
    ...
    const debugStopRoute = {
        ...
        handler: async (req, res) => {
            ...
            if (parsed.sessionId === undefined)
                return json(res, { error: 'sessionId 必填' }, 400);
    ...
    ctx.effect(() => () => {
        ...
        for (const st of thinkStreams.values())
            if (st.timer !== undefined)
                clearTimeout(st.timer);
        thinkStreams.clear();
        lspFeedback.dispose();
        lsp.dispose();
        debug.dispose();
        for (const t of queueSnapTimers.values())
...
// ---- pairing helpers ----
function buildPairInfo(req, pairToken, port, bindHost, lanPort) {
    const ips = lanIpv4s();
...
function headerHostIp(hostHeader) {
    if (!hostHeader)
        return null;
...
function renderPairPage(info, svg, loopbackOnly) {
...
function deliveryCompleteBody(sessionTitle, isSubagent) {
    const name = sessionTitle.trim();
    if (isSubagent)
        return name ? `「${name}」已完成` : '子代理已完成';
...
function toolFilePaths(data) {
    if (data.name !== 'edit' && data.name !== 'write' && data.name !== 'str_replace_editor')
        return [];
 ...
 */
function projectWindowBack(ctx, all, beforeSeq, limit, scope) {
    const events = [];
    let idx = all.length - 1;
    if (beforeSeq !== undefined) {
        while (idx >= 0 && all[idx].seq >= beforeSeq)
            idx--;
    }
    while (idx >= 0) {
...
function projectEvent(ctx, event, scope) {
    ...
    if (rawType === 'command/run') {
        ...
        const d = event.data;
        if (d === undefined || typeof d.commandId !== 'string')
            return [];
        ...
        if (name !== undefined) {
            if (commandMeta.size >= 200) {
                ...
                if (!first.done)
                    commandMeta.delete(first.value);
            }
    ...
    switch (event.type) {
        case 'user/message': {
            ...
            const source = (src === 'user' || src === 'coordinator') ? 'user' : 'inject';
        ...
        case 'assistant/message': {
            ...
            if (text)
                out.push({ ...base, type: 'assistant_message', text });
            return out;
        }
        case 'tool/call': {
            ...
            try {
                ...
                const view = ctx.tools.get(String(event.data.name), scope)?.presentCall?.(args);
                if (view !== undefined) {
                    toolCard = view.card;
                    ...
                    if ('kind' in view && view.kind !== undefined)
                        toolKind = view.kind;
        ...
        case 'tool/result': {
            ...
            const cid = data.message?.content
                ?.find((b) => b?.toolCallId !== undefined)
                ?.toolCallId;
            return [{
                    ...base,
                    type: 'tool_result',
