...
export function apply(ctx) {
    ...
    const listSessions = () => ctx.sessions.list().map((s) => {
        ...
        return {
            ...
            agentCount: 1,
            subagentCount: allAgents().filter((x) => String(x.session.header.parentSession) === String(s.id) && isSubagent(x)).length,
    ...
    const ping = {
        kind: 'exact',
        path: '/remote/ping',
        handler: (_req, res) => json(res, { ok: true, version: BRIDGE_VERSION, serverId, hostname: host, sessions: listSessions().length }),
    };
    const health = {
        kind: 'exact',
        path: '/remote/health',
