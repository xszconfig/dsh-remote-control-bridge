...
export function apply(ctx) {
    ...
    // elapsed 以"本轮"起点（turnStarts）计：跨多次模型调用连续累计，不随每次请求归零。
    const divingTicker = setInterval(() => {
        for (const [sid, startedAt] of modelStreams) {
            const base = turnStarts.get(sid) ?? startedAt;
            broadcast({
    ...
    ctx.on('session/event', (session, event) => {
        ...
        if (event.type === 'turn/start') {
            try {
                ...
                logger.warn('TODO', `turn/start 清空广播失败 session=${String(session.id).slice(0, 12)}: ${String(e)}`);
            }
