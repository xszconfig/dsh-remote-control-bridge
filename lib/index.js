        ...
        switch (cmd.type) {
            ...
            case 'subscribe': {
                ...
                try {
                    ...
                    const { events } = await ctx.sessionPersistence.readFrom(SessionId(cmd.sessionId), 0);
                    const history = events
                        .map(projectEvent)
                        .filter((e) => e !== null);
                    logger.info('WS', `冷会话历史 session=${cmd.sessionId.slice(0, 12)} events=${history.length}`);
                    send(ws, { type: 'history', sessionId: cmd.sessionId, events: history });
                }
                catch (e) {
