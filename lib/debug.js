...
class InspectorSession {
    ...
    pausedFrames = [];
    pauseReason = '';
    breakpointsDone = false; // 断点注册完才恢复入口暂停（避免入口暂停先恢复导致断点竞态）
    entryArmed = false; // runIfWaitingForDebugger 后的首个 paused 是 -brk 入口暂停，自动恢复一次
    stopped = false;
    outputRing = [];
    consoleLines = new Set(); // consoleAPICalled 与 stdout 双通道去重
    failReason = null; // 启动失败/协议错误的说明（正常退出为 null）
    connectTimer = null; // 连接 Inspector 的 10s 兜底超时
    outputTimers = new Set(); // stdout 侧 250ms 去重延迟（卸载时统一清理）
    constructor(hooks, sessionId, program, cwd, breakpoints) {
        this.hooks = hooks;
    ...
    snapshot(pausedFrames) {
        const state = pausedFrames !== undefined ? 'paused' : (this.stopped ? 'stopped' : (this.ws ? 'running' : 'starting'));
        const snap = {
            state,
    ...
    wireFrame(f) {
        ...
        return {
            ...
            scopes: f.scopeChain
                ...
                variablesReference: s.object.objectId,
            })),
        };
    }
    start() {
        ...
        child.on('exit', (code) => {
            if (!this.stopped) {
                this.stopped = true;
                this.rejectPending(new Error('调试进程已退出'));
                this.ws?.close();
                this.ws = null;
        ...
        this.connectTimer = setTimeout(() => {
            ...
            if (this.ws === null && !this.stopped)
                this.failStart('连接调试器超时（未能从 stderr 发现 Inspector 地址）');
        }, 10_000);
    }
    failStart(message) {
        ...
        return new Promise((resolveConn) => {
            ...
            ws.on('open', () => {
                void this.send('Runtime.enable', {})
                    ...
                    .catch((e) => this.failStart(`调试协议初始化失败: ${String(e)}`));
                resolveConn();
            });
            ws.on('message', (raw) => {
                try {
            ...
            ws.on('error', (e) => {
                if (!this.stopped)
                    this.failStart(`调试器连接错误: ${e.message}`);
            });
            ws.on('close', () => {
                this.ws = null;
                this.rejectPending(new Error('调试器连接已关闭'));
                if (!this.stopped) {
                    this.stopped = true;
                    this.hooks.onState(this.sessionId, this.snapshot());
                }
    ...
    /**
     * 拒绝所有在途 CDP 请求。根因修复：旧实现只 `pending.clear()`，导致这些 Promise
     * 永不落定、await 方（variablesFor/stop）无限挂起（与 lsp_query 卡死同款缺陷）。
     */
    rejectPending(err) {
    ...
    emitOutput(line) {
        ...
        if (this.outputRing.length > OUTPUT_CAP)
    ...
    async command(action) {
        ...
        try {
            ...
            // 调试器已断开/进程已退出：忽略（状态经 onState 推送）
        }
    }
    /** 读取变量并广播到手机（手机按钮走这条）。 */
    ...
    async variablesFor(variablesReference) {
        if (this.ws === null || this.stopped)
            return [];
        try {
            ...
            return result
                ...
                return {
                    ...
                    type,
                    hasChildren,
                    variablesReference: hasChildren ? v.objectId : '',
                };
            });
        }
        catch {
    ...
    dispose() {
        ...
        for (const t of this.outputTimers)
            clearTimeout(t);
        this.outputTimers.clear();
        // 拒绝在途 CDP 请求（根因修复：旧实现只 clear 导致 await 方无限挂起；
        // 各 await 方已带 try/catch 或 void 前自捕获，不产生未处理拒绝）。
...
export class DebugManager {
    ...
    command(sessionId, action) {
        const s = this.sessions.get(sessionId);
        if (s === undefined)
            throw new Error('该会话没有调试进程');
    ...
    async variablesFor(sessionId, variablesReference) {
        const s = this.sessions.get(sessionId);
        if (s === undefined)
            throw new Error('该会话没有调试进程');
