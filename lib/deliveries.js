...
export function loadDeliveries(file) {
    try {
        ...
        if (parsed === null || typeof parsed !== 'object' || !Array.isArray(parsed.deliveries))
            return [];
        const out = [];
        for (const it of parsed.deliveries) {
            ...
            if (typeof r.sessionId !== 'string' || typeof r.turnKey !== 'string')
                continue;
            out.push({
...
export function writeDeliveries(file, records) {
