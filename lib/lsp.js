...
/** Agent 查询工具的单次 deadline：服务器假死/已死时绝不无限挂起（生产事故曾卡死 agent 回合 400+ 分钟）。 */
const LSP_QUERY_TIMEOUT_MS = 10_000;
 ...
 */
export function kotlinProjectRoot(path) {
    const start = dirname(path);
    const withSettings = [];
    const withAny = [];
    let dir = start;
    for (let i = 0; i < KOTLIN_ROOT_MAX_DEPTH; i++) {
        if (KOTLIN_SETTINGS_MARKERS.some((m) => existsSync(join(dir, m))))
            withSettings.push(dir);
        if (KOTLIN_ROOT_MARKERS.some((m) => existsSync(join(dir, m))))
            withAny.push(dir);
 ...
 */
export function kotlinSpawnEnv(base, opts) {
    const env = { ...base };
    for (const k of KOTLIN_STRIP_PROXY_VARS)
        delete env[k];
    if (env.GRADLE_USER_HOME === undefined || env.GRADLE_USER_HOME === '')
        env.GRADLE_USER_HOME = opts.gradleUserHome;
    if (opts.javaHome !== undefined && opts.javaHome !== '' && (env.JAVA_HOME === undefined || env.JAVA_HOME === ''))
...
export class LspManager {
    ...
    async query(action, path, line, column) {
        ...
        // 快速失败：进程已退出/被杀/未存活，直接返回明确文本，绝不进入任何可能无限挂起的 await
        if (this.serverDead(state))
        ...
        catch { /* 文件可能暂不存在，保留原路径 */ }
        const uri = pathToFileURL(realPath).href;
        return await withTimeout(this.queryInner(lang, state, action, path, realPath, uri, line, column), LSP_QUERY_TIMEOUT_MS, () => ({ text: `LSP 查询超时（${LSP_QUERY_TIMEOUT_MS / 1000}s）：${lang} 语言服务器无响应，可能未启动或已退出` }));
    }
    /** query 的 deadline 内主体：初始化等待 + 具体查询（在 query() 里被 10s 兜底，绝不无限挂起）。 */
    async queryInner(lang, state, action, path, realPath, uri, line, column) {
        try {
    ...
    gradleJavaHome() {
        ...
        if (fromEnv !== undefined && fromEnv !== '' && existsSync(join(fromEnv, 'bin', 'java')))
            return fromEnv;
        const candidates = [
        ...
        try {
            ...
            proc.on('exit', () => {
                if (!state.dead) {
                    ...
                    this.rejectPending(state, new Error('server exited'));
                }
            });
    ...
    killServer(state) {
        ...
        this.rejectPending(state, new Error('server killed'));
    }
    /** 进程是否已死/被杀/未存活（含 exit 事件尚未触发、仅 exitCode 已置位的窗口期）。 */
    serverDead(state) {
        return state.dead || state.proc.exitCode !== null || state.proc.signalCode !== null || state.proc.killed;
    }
    /**
    ...
    syncDoc(state, uri, path) {
        ...
        if (state.pullDiagnostics)
            this.pullAllDiagnostics(state);
    }
    /** pull 模式：同步后拉取该 server 全部已打开文档的诊断（跨文件覆盖）。 */
    pullAllDiagnostics(state) {
